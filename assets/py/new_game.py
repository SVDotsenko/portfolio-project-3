...
def new_game(human_board, computer_board):
    ...
    print("Player hit this time.") if guess(computer_board, row - 1, col - 1) else print("Player missed this time.")

    (row, col) = computer_board.computer_guesses.pop().popitem()

    print(f'Computer guessed: ({row + 1}, {col + 1})')
    print("Computer hit this time.") if guess(human_board, row, col) else print("Computer missed this time.")
